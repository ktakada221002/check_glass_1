        ...
        try:
            while True:
                if ctypes.windll.user32.GetAsyncKeyState(0x01) == 0x8000:
                    ...
                    # Entryウィジェットへ文字列のセット
                    txt_1.insert(tkinter.END, x)

                    lbl_2 = tkinter.Label(text="Y座標")
                    lbl_2.place(x=30, y=140)
                    txt_2 = tkinter.Entry(width=20)
                    txt_2.place(x=140, y=140)
                    txt_2.insert(tkinter.END, y)

                    lbl_3 = tkinter.Label(text="欠点ｻｲｽﾞ")
                    lbl_3.place(x=30, y=210)
                    txt_3 = tkinter.Entry(width=20)
                    txt_3.place(x=140, y=210)

                    lbl_4 = tkinter.Label(text="ID")
                    lbl_4.place(x=30, y=0)
                    txt_4 = tkinter.Entry(width=20)
                    txt_4.place(x=140, y=0)

                    # ボタン作
                    btn_1 = tkinter.Button(root, text="入力", command=btn_click_1)
                    btn_1.place(x=50, y=300)

                    btn_2 = tkinter.Button(root, text="終了", command=btn_click_2)
                    btn_2.place(x=250, y=300)

                    btn_3 = tkinter.Button(root, text="やり直し", command=btn_click_3)
                    btn_3.place(x=100, y=300)

                    btn_4 = tkinter.Button(root, text="表示削除", command=btn_click_4)
